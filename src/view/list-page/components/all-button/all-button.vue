<template>
  <div>
    ...
    <div style="display: inline-block;" v-show="item.Btn_Type !='3'" v-for="(item, index) in btnData" :key="index">
      <el-button @click="btnClick(item)" v-if="item.Btn_Type !='6'" class="btn" :class="item.color">{{ item.Btn_Text }}</el-button>
      <el-upload accept=".xls,.xlsx" :name="fileName" :action="elexUrl"  v-if="item.Btn_Type =='6'" :show-file-list="false" :on-success="fileSuccess" :on-error="fileError">
        <el-button @click="btnClick(item)" class="btn" :class="item.color">{{ item.Btn_Text }}</el-button>
...
export default {
  ...
  watch: {
    btnData(n, o) {
      ...
      this.btnData.forEach((item) => {
        ...
        if (item.Btn_Type == "1" || item.Btn_Type == "0" || item.Btn_Type == "3" || item.Btn_Type == "7" || item.Btn_Type == "6") {
  ...
  methods: {
    ...
    importExcelData(item){
      ...
      this.elexUrl = `${dev}/Common/Common/ImportExcel?Page_ID=${Page_ID}`
