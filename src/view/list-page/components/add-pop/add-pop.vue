    ...
    <div v-drag1 class="scPopBox_c" ref="scPopBoxC">
      ...
      >
        <div class="pop-centent">
          ...
          >
            <span class="pop-text" style="float: left"
              >{{ item.fieldname }}:</span
            >
            <div
              class="elemen_box"
              :class="item.fieldtype == 'textarea' ? 'textB' : ''"
            >
              <el-input
                :disabled="item.isupdateavailable != '1'"
                @input="inputFun(item)"
                v-if="item.fieldtype == 'num'"
                v-model="item.value"
                placeholder="请输入内容"
              ></el-input>
              <el-autocomplete
  ...
  methods: {
    ...
    selectDataFun(arr) {
      for (let j = 0; j < arr.length; j++) {
        var selectId;
        for (let k = 0; k < this.dataArr.length; k++) {
          //获取选择的id
          ...
          if (index != -1) {
            for (let j = 0; j < this.GroupSelectAll.length; j++) {
              if (this.dataArr[i].fieldID == this.GroupSelectAll[j].id) {
                ...
                for (
                  let k = 0;
                  k < this.GroupSelectAll[j].selection.length;
                  k++
                ) {
          ...
          if (item == this.dataArr[i].fieldindex) {
            if (this.axioData.WindowType != 3) {
              ...
              if (this.dataArr[i].fieldtype == "selection") {
                ...
                if (this.dataArr[i].IsGroup != "undefined") {
                  if (this.dataArr[i].IsGroup == 1) {
                    await this.GroupSelectFun(this.dataArr[i].fieldID).then(
                      (val) => {
                        this.dataArr[i].data = val.data;
                        this.dataArr[i].value = this.dataArr[i].data[0].text;
                        this.dataArr[i].childrenName = val.id;
                    ...
                    await this.selectFun(this.dataArr[i].Selection_Url).then(
                      (val) => {
                        // console.log(val)
                        this.dataArr[i].data = val;
