        ...
        <div class="shanhe_conter clearfix">
            ...
            <div class="shanhe_middle left">
                <div class="middle_top">
                    <el-table height="90%" class="middleTable" :data="tableData" :row-class-name="tableRowClassName"
                        :header-cell-style="headClass" style="width: 96%">
                        <el-table-column prop="ID" :show-overflow-tooltip="true" label="任务号" width="140">
                        </el-table-column>
                        <el-table-column prop="TrayID" :show-overflow-tooltip="true" width="145" label="托盘号">
                        </el-table-column>
                        <el-table-column prop="EntryChildID" :show-overflow-tooltip="true" width="265" label="胶囊型号">
                        </el-table-column>
                        <el-table-column prop="CommandState" :show-overflow-tooltip="true" label="任务状态" width="150">
                        </el-table-column>
                        <el-table-column prop="CommandType" :show-overflow-tooltip="true" label="任务类型" width="140">
                        </el-table-column>
                    </el-table>
            ...
            <div class="shanhe_right left">
                ...
                <div class="right_top">
                    ...
                    <div class="right_top_down">
                        <div class="joinLogo"></div>
                        <div class="joinText">胶囊库存总数</div>
                        <div class="border">
                            <div class="joinNum Num1">{{ jiao1 }}</div>
                            <div class="joinNum Num2">{{ jiao2 }}</div>
                            <div class="joinNum Num3">{{ jiao3 }}</div>
                            <div class="joinNum Num4">{{ jiao4 }}</div>
                            <div class="joinNum Num5">{{ jiao5 }}</div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        getTable() {
            this.$axios({
                method: 'get',
                url: `/WMSBusinessManage/Kanban/GetCommandList`,
            }).then(res => {
                if (res.data.type == 1) {
        ...
        getAmount() {
            this.$axios({
                method: 'get',
                url: `/WMSBusinessManage/Kanban/GetContainerRatio`,
            }).then(res => {
                if (res.data.type == 1) {
                    const data = JSON.parse(res.data.resultdata)[0]
                    this.can1 = 0
                    this.can2 = 0
                    this.can3 = 0
                    this.can4 = 0
                    ...
                    this.use4 = 0
                    this.use5 = 0
                    var Free = data.UsableCount.toString()
                    var FullC = data.UnusableCount.toString()
        ...
        GetGuZhang() {
            this.$axios({
                method: 'get',
                url: `/WMSBusinessManage/Kanban/GetTrayStockTotal`,
            }).then(res => {
                if (res.data.type == 1) {
                    const data = JSON.parse(res.data.resultdata)[0]
                    this.jiao1 = 0
                    ...
                    this.jiao4 = 0
                    this.jiao5 = 0
                    var Free = data.MaterialAmountTotal.toString()
                    for (let i = 0; i < Free.length; i++) {
                        var Fvalue = Free.slice(Free.length - (1 + i), Free.length - i)
                        i == 0 ? this.jiao5 = Fvalue : i == 1 ? this.jiao4 = Fvalue : i == 2 ? this.jiao3 = Fvalue : i == 3 ? this.jiao2 = Fvalue : i == 4 ? this.jiao1 = Fvalue : this.jiao1 = Fvalue
                    }
        ...
        GetEDOut() {
            this.$axios({
                method: 'get',
                url: `/WMSBusinessManage/Kanban/GetCommandTotelByToday?currDate=${this.$moment().format('YYYY-MM-DD')}`,
            }).then(res => {
                if (res.data.type == 1) {
                    const data = JSON.parse(res.data.resultdata)[0]
                    this.EntryAmount = data.EntryCount;
                    this.DeliverAmount = data.DeliverCount;
        ...
        GetEnterAmount() {
            this.$axios({
                method: 'get',
                url: `/WMSBusinessManage/Kanban/GetEntryStatistics?commandType=1`,
            }).then(res => {
                if (res.data.type == 1) {
                    const data = JSON.parse(res.data.resultdata)
                    if (data.length) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            time.push(data[i].GroupDay)
                            value.push(data[i].TotalCount)
                        }
        ...
        GetDeliverAmount() {
            this.$axios({
                method: 'get',
                url: `/WMSBusinessManage/Kanban/GetEntryStatistics?commandType=2`,
            }).then(res => {
                if (res.data.type == 1) {
                    const data = JSON.parse(res.data.resultdata)
                    if (data.length) {
...
.shanhe_box {
    ...
    .shanhe_head {
        ...
        .shanhe_title {
            ...
            width: 400px;
            height: 54px;
            background: url('../../assets/img/kanban_title.png');
            background-size: cover;
            ...
            .right_top {
                ...
                .right_top_down {
                    ...
                    .border {
                        height: 100%;
                        width: calc(100% - 50px);
                        border-bottom: 1px solid #01EDFE;
                        position: absolute;
                        bottom: 0;
                        left: 50%;
